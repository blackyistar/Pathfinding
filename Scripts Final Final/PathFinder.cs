...
public class PathFinder : MonoBehaviour
{
    ...
    public void ClearPaths() //Breaks the algorithms and clears all completed paths from tile map and clears the leader board results
    {
    ...
    public void ClearAllGridResults() //Clears all completed paths from tile map and clears the leaderboard results
    {
        foreach (GridController grid in grids)
        {
            grid.ClearResults();
    ...
    public void UpdateAllGrids() //Updates the colours of the tiles on every tile map
    {
        foreach (GridController grid in grids)
        {
            grid.UpdateGrid(null);
    ...
    public bool SetTarget() //Sets the target node for pathfinding on every tile map
    {
        ...
        {
            if (hit.transform.gameObject.layer != grids[0].unwalkableLayer)
            {
    ...
    public Node SetSecondaryTarget(Vector3 primaryTarget, GridController primaryGrid, GridController secondaryGrid) //Translates the position of a tile on a tilemap to the position of the same tile on another tile map
    {
    ...
    public IEnumerator DepthFirst(GridController grid) //Performs Depth first search and is a coroutine to allow for execution to be suspendeded
    {
        float timer = 0;
        ...
        while (stack.Count != 0)
        {
            ...
            Node up = null, right = null, down = null, left = null;
            ...
            List<Node> ordered = new List<Node> { up, right, down, left };
    ...
    public IEnumerator BreadthFirst(GridController grid) //Performs Breadth first search and is a coroutine to allow for execution to be suspendeded
    {
        float timer = 0;
    ...
    public IEnumerator Dijkstras(GridController grid) //Performs Dijkstra's and is a coroutine to allow for execution to be suspendeded
    {
        float timer = 0;
    ...
    public IEnumerator AStar(GridController grid) //Performs A star search and is a coroutine to allow for execution to be suspendeded
    {
        float timer = 0;
